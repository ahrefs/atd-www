...
let expect_error f x =
  try
    ...
    fail ()
  with
    Atdgen_extra_runtime.Ob_run.Error _
  | Atdgen_extra_runtime.Oj_run.Error _ -> ()
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0xf then Atdgen_extra_runtime.Ob_run.missing_fields
          [| !bits0 |] [| "b0"; "b1"; "b2"; "b4" |];
        (
          {
            b0x = !field_b0x;
...
let test_json_files () =
  section "json files";
  let x = Some 123 in
  let s = Atdgen_extra_runtime.Util.Json.to_string Testj.write_intopt x in
  let x' = Atdgen_extra_runtime.Util.Json.from_string Testj.read_intopt s in
  check (x = x');
  Atdgen_extra_runtime.Util.Json.to_file Testj.write_intopt "test-json-files.json" x;
  let x'' = Atdgen_extra_runtime.Util.Json.from_file Testj.read_intopt "test-json-files.json" in
  check (x = x'')

let test_json_streams () =
  section "json streams";
  let l = [ Some 1; None; Some 2; Some 3 ] in
  let s = Atdgen_extra_runtime.Util.Json.list_to_string Testj.write_intopt l in
  let l' = Atdgen_extra_runtime.Util.Json.list_from_string Testj.read_intopt s in
  check (l = l');
  Atdgen_extra_runtime.Util.Json.list_to_file Testj.write_intopt "test-json-streams.json" l;
  let l'' =
    Atdgen_extra_runtime.Util.Json.list_from_file Testj.read_intopt "test-json-streams.json"
  in
  check (l = l'')
