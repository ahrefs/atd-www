...
type mixed =
  (
      mixed_record Atdgen_extra_runtime.Util.ocaml_array
    * mixed_record Atdgen_extra_runtime.Util.ocaml_array
  ) list
...
type int_assoc_array = (string * int) Atdgen_extra_runtime.Util.ocaml_array
...
type 'a array = 'a Atdgen_extra_runtime.Util.ocaml_array
...
let write_untagged__19 _a_tag write_untagged__a write__a = (
  Atdgen_extra_runtime.Ob_run.write_untagged_list
    _a_tag
    (
      write_untagged__a
    )
...
let get__19_reader get__a_reader read__a = (
  Atdgen_extra_runtime.Ob_run.get_list_reader (
    get__a_reader
  )
)
let read__19 get__a_reader read__a = (
  Atdgen_extra_runtime.Ob_run.read_list (
    get__a_reader
  )
)
...
let rec get_p'_reader get__a_reader read__a = (
  fun tag ->
    if tag <> 23 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        Bi_io.read_hashtag ib (fun ib h has_arg ->
          match h, has_arg with
            ...
            | _ -> Atdgen_extra_runtime.Ob_run.unsupported_variant h has_arg
        )
)
and read_p' get__a_reader read__a = (
  fun ib ->
    if Bi_io.read_tag ib <> 23 then Atdgen_extra_runtime.Ob_run.read_error_at ib;
    Bi_io.read_hashtag ib (fun ib h has_arg ->
      match h, has_arg with
        | 65, false -> (A : 'a p')
        | 14816, true -> (Bb (
...
and get_r_reader = (
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        for i = 1 to len do
          match Bi_io.read_field_hashtag ib with
            | 97 ->
              field_a := (
                (
                  Atdgen_extra_runtime.Ob_run.read_int
                ) ib
              );
              bits0 := !bits0 lor 0x1;
            | 98 ->
              field_b := (
                (
                  Atdgen_extra_runtime.Ob_run.read_bool
                ) ib
              );
              bits0 := !bits0 lor 0x2;
            | 99 ->
        ...
        if !bits0 <> 0x7 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "a"; "b"; "c" |];
        (
          {
            a = !field_a;
            b = !field_b;
...
and read_r = (
  fun ib ->
    if Bi_io.read_tag ib <> 21 then Atdgen_extra_runtime.Ob_run.read_error_at ib;
    let field_a = ref (Obj.magic (Sys.opaque_identity 0.0)) in
    let field_b = ref (Obj.magic (Sys.opaque_identity 0.0)) in
    let field_c = ref (Obj.magic (Sys.opaque_identity 0.0)) in
    let bits0 = ref 0 in
...
and write_untagged__20 _a_tag write_untagged__a write__a _b_tag write_untagged__b write__b ob x = (
  Atdgen_extra_runtime.Ob_run.write_untagged_option (
...
let rec get__20_reader get__a_reader read__a get__b_reader read__b = (
  fun tag ->
    if tag <> 22 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        match Char.code (Bi_inbuf.read_char ib) with
          ...
          | _ -> Atdgen_extra_runtime.Ob_run.read_error_at ib
)
and read__20 get__a_reader read__a get__b_reader read__b = (
  fun ib ->
    if Bi_io.read_tag ib <> 22 then Atdgen_extra_runtime.Ob_run.read_error_at ib;
    match Char.code (Bi_inbuf.read_char ib) with
      | 0 -> None
      | 0x80 ->
        Some (
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0x3 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "fst"; "snd" |];
        (
          {
            fst = !field_fst;
            snd = !field_snd;
        ...
        Bi_io.read_hashtag ib (fun ib h has_arg ->
          match h, has_arg with
            ...
            | -733074589, true -> (`Case3 (
                (
                  Atdgen_extra_runtime.Ob_run.read_string
                ) ib
              ))
...
let get_validated_string_check_reader = (
  Atdgen_extra_runtime.Ob_run.get_string_reader
)
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0x1 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "val1_x" |];
        (
          {
            val1_x = !field_val1_x;
          }
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0x1 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "val2_x" |];
        (
          {
            val2_x = !field_val2_x;
            val2_y = !field_val2_y;
...
let get__29_reader = (
  Atdgen_extra_runtime.Ob_run.get_list_reader (
    Atdgen_extra_runtime.Ob_run.get_float64_reader
  )
)
let read__29 = (
  Atdgen_extra_runtime.Ob_run.read_list (
...
let get_date_reader = (
  fun tag ->
    if tag <> 20 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        let len = Bi_vint.read_uvint ib in
        if len < 3 then Atdgen_extra_runtime.Ob_run.missing_tuple_fields len [ 0; 1; 2 ];
        let x0 =
          (
            Atdgen_extra_runtime.Ob_run.read_int
          ) ib
...
let read_date = (
  fun ib ->
    if Bi_io.read_tag ib <> 20 then Atdgen_extra_runtime.Ob_run.read_error_at ib;
    let len = Bi_vint.read_uvint ib in
    if len < 3 then Atdgen_extra_runtime.Ob_run.missing_tuple_fields len [ 0; 1; 2 ];
    let x0 =
      (
...
let write_untagged__9 = (
  Atdgen_extra_runtime.Ob_run.write_untagged_array
    Bi_io.string_tag
    (
      Bi_io.write_untagged_string
    )
...
let string_of__9 ?(len = 1024) x =
  ...
  Bi_outbuf.contents ob
let get__9_reader = (
  Atdgen_extra_runtime.Ob_run.get_array_reader (
    Atdgen_extra_runtime.Ob_run.get_string_reader
  )
)
let read__9 = (
  Atdgen_extra_runtime.Ob_run.read_array (
    Atdgen_extra_runtime.Ob_run.get_string_reader
  )
)
      ...
      fun ib ->
        match Char.code (Bi_inbuf.read_char ib) with
          ...
          | 0x80 ->
            Some (
              (
                Atdgen_extra_runtime.Ob_run.read_float64
              )
                ib
            )
          | _ -> Atdgen_extra_runtime.Ob_run.read_error_at ib
...
let get__10_reader = (
  Atdgen_extra_runtime.Ob_run.get_list_reader (
    Atdgen_extra_runtime.Ob_run.get_unit_reader
  )
)
let read__10 = (
  Atdgen_extra_runtime.Ob_run.read_list (
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        for i = 1 to len do
          match Bi_io.read_field_hashtag ib with
            ...
            | 9342521 ->
              field_field3 := (
                (
                  Atdgen_extra_runtime.Ob_run.read_int64
                ) ib
              );
              bits0 := !bits0 lor 0x2;
            | 9342522 ->
              ...
              field_field9 := (
                (
                  fun ib ->
                    if Bi_io.read_tag ib <> 20 then Atdgen_extra_runtime.Ob_run.read_error_at ib;
                    let len = Bi_vint.read_uvint ib in
                    if len < 6 then Atdgen_extra_runtime.Ob_run.missing_tuple_fields len [ 0; 1; 2; 3; 4; 5 ];
                    let x0 =
                      (
                        Atdgen_extra_runtime.Ob_run.read_int
                      ) ib
                    ...
                    let x2 =
                      (
                        Atdgen_extra_runtime.Ob_run.read_char
                      ) ib
                    in
                    let x3 =
                      (
                    ...
                    let x4 =
                      (
                        Atdgen_extra_runtime.Ob_run.read_int32
                      ) ib
                    in
                    let x5 =
                      (
        ...
        if !bits0 <> 0x3ff then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "field2"; "field3"; "field4"; "field7"; "field8"; "field9"; "field10"; "field12"; "field13"; "field14" |];
        (
          {
            field0 = !field_field0;
            field1 = !field_field1;
...
let write_untagged__12 = (
  fun ob x ->
    ...
    if len > 0 then (
      ...
      Atdgen_extra_runtime.Ob_run.array_iter2 (fun ob x ->
        (
          write_untagged__4
        )
          ob x.field0;
...
let get__12_reader = (
  function
    | 25 -> 
      (fun ib ->
        ...
        if row_num = 0 then [| |]
          ...
          let bits0 = ref 0 in
          let readers =
            Atdgen_extra_runtime.Ob_run.array_init2 col_num ib (
              fun col ib ->
                ...
                match h with
                  ...
                  | 9342521 ->
                    ...
                    let read =
                      (
                        Atdgen_extra_runtime.Ob_run.get_int64_reader
                      )
                        tag
                    in
                  ...
                  | -64101863 ->
                    ...
                    let read =
                      (
                        Atdgen_extra_runtime.Ob_run.get_bool_reader
                      )
                        tag
                    in
    ...
    | 19 -> 
      (fun ib ->
        Atdgen_extra_runtime.Ob_run.read_array_value (
          get_mixed_record_reader
        ) ib
      )
    | _ -> Atdgen_extra_runtime.Ob_run.read_error ()
)
let read__12 = (
  fun ib ->
  ...
  Atdgen_extra_runtime.Ob_run.get_list_reader (
    fun tag ->
      if tag <> 20 then Atdgen_extra_runtime.Ob_run.read_error () else
        fun ib ->
          let len = Bi_vint.read_uvint ib in
          if len < 2 then Atdgen_extra_runtime.Ob_run.missing_tuple_fields len [ 0; 1 ];
          let x0 =
            (
              read__12
            ) ib
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0x7 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "x2"; "x3"; "x4" |];
        (
          {
            x0 = !field_x0;
            x1 = !field_x1;
...
let get_star_rating_reader = (
  Atdgen_extra_runtime.Ob_run.get_int_reader
)
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0x1 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "x294623" |];
        (
          {
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0x1 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "some_field" |];
        (
          {
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0x7 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "sqrt2_5"; "small_2"; "large_2" |];
        (
          {
...
let get_int32_reader = (
  Atdgen_extra_runtime.Ob_run.get_int32_reader
)
let read_int32 = (
  Atdgen_extra_runtime.Ob_run.read_int32
)
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        for i = 1 to len do
          match Bi_io.read_field_hashtag ib with
            | 5083781 ->
              field_f32 := (
                (
                  Atdgen_extra_runtime.Ob_run.read_float32
                ) ib
              );
              bits0 := !bits0 lor 0x1;
            | 5084452 ->
        ...
        if !bits0 <> 0x3 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "f32"; "f64" |];
        (
          {
            f32 = !field_f32;
            f64 = !field_f64;
...
let get_extended_tuple_reader = (
  fun tag ->
    if tag <> 20 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        let len = Bi_vint.read_uvint ib in
        if len < 5 then Atdgen_extra_runtime.Ob_run.missing_tuple_fields len [ 0; 1; 4 ];
        let x0 =
          (
            Atdgen_extra_runtime.Ob_run.read_int
          ) ib
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0xf then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "b0"; "b1"; "b2"; "b4" |];
        (
          {
            b0x = !field_b0x;
            b1x = !field_b1x;
...
let string_of_char ?(len = 1024) x =
  ...
  Bi_outbuf.contents ob
let get_char_reader = (
  Atdgen_extra_runtime.Ob_run.get_char_reader
)
let read_char = (
  Atdgen_extra_runtime.Ob_run.read_char
)
  ...
  fun tag ->
    if tag <> 21 then Atdgen_extra_runtime.Ob_run.read_error () else
      fun ib ->
        ...
        if !bits0 <> 0x3 then Atdgen_extra_runtime.Ob_run.missing_fields [| !bits0 |] [| "b0"; "b1" |];
        (
          {
            b0 = !field_b0;
            b1 = !field_b1;
