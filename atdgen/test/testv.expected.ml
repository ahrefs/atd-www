...
let validate__19 validate__a = (
  Atdgen_extra_runtime.Ov_run.validate_list (
    validate__a
  )
)
...
let rec validate_p = (
  fun path x ->
    match ( fun path x ->
         match x with
           `A -> Some (Atdgen_extra_runtime.Util.Validation.error path)
         | _ -> None ) path x with
      | Some _ as err -> err
      | None ->
        match x with
...
let rec validate__20 validate__a validate__b path x = (
  Atdgen_extra_runtime.Ov_run.validate_option (
...
let validate_validated_string_check = (
  fun path x ->
    ...
    if (fun s -> s = "abc") x then
      ...
      Some (Atdgen_extra_runtime.Util.Validation.error ~msg path)
)
let validate__31 = (
  (fun path x ->
    (match ( fun path x ->
    ...
    if (fun l -> true) x then
      ...
      Some (Atdgen_extra_runtime.Util.Validation.error ~msg path) ) path x with
      | Some _ as err -> err
      | None -> (
          Atdgen_extra_runtime.Ov_run.validate_list (
            fun path x ->
...
let validate_val1 : _ -> val1 -> _ = (
  fun path x ->
    (
      fun path _ -> Some (Atdgen_extra_runtime.Util.Validation.error path)
...
let validate_extended : _ -> extended -> _ = (
  fun path x ->
    match ( fun path x ->
      ...
      else Some (Atdgen_extra_runtime.Util.Validation.error path) ) path x with
      | Some _ as err -> err
      | None ->
        (
          fun path x ->
        if x = false then None else Some (Atdgen_extra_runtime.Util.Validation.error path)
...
let validate_base_tuple = (
  fun path (i, f) -> if float i < f then None else Some (Atdgen_extra_runtime.Util.Validation.error ~msg:"i < f" path)
)
let validate_base : _ -> base -> _ = (
  fun _ _ -> None
)
let validate__23 validate__a = (
  Atdgen_extra_runtime.Ov_run.validate_array (
    validate__a
  )
)
