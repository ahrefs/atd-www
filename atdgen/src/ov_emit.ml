  ...
  |> List.iter (fun x ->
    ...
    let validator_params =
      String.concat "" (
        List.map
          (fun s ->
             sprintf "\n  (Atdgen_extra_runtime.Util.Validation.path -> '%s -> \
                      Atdgen_extra_runtime.Util.Validation.error option) ->" s)
          x.def_param
      )
    in
    let s = x.def_name in
    if Ox_emit.is_exportable x then (
      bprintf buf "\
val validate_%s :%s
  Atdgen_extra_runtime.Util.Validation.path -> %s -> Atdgen_extra_runtime.Util.Validation.error option
...
let rec make_validator (x : ov_mapping) : Indent.t list =
  ...
  | List (_, x, List o, (v, shallow)) ->
      if shallow then
        ...
        let validate =
          match o with
            List -> "Atdgen_extra_runtime.Ov_run.validate_list ("
          | Array -> "Atdgen_extra_runtime.Ov_run.validate_array ("
        in
  ...
  | Nullable (_, x, Nullable, (v, shallow)) ->
      if shallow then
        ...
        prepend_validator_f v [
          Line "Atdgen_extra_runtime.Ov_run.validate_option (";
          Block (make_validator x);
          Line ")";
        ]
