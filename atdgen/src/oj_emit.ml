...
let rec get_writer_name
  ...
  | Int (_, Int o, Int) ->
      (match o with
       ...
       | Char ->  "Atdgen_extra_runtime.Oj_run.write_int8"
       | Int32 -> "Atdgen_extra_runtime.Oj_run.write_int32"
       | Int64 -> "Atdgen_extra_runtime.Oj_run.write_int64"
       | Float -> "Atdgen_extra_runtime.Oj_run.write_float_as_int"
      )
...
let rec get_reader_name
  ...
  match x with
    Unit (_, Unit, Unit) -> "Atdgen_extra_runtime.Oj_run.read_null"
  | Bool (_, Bool, Bool) -> "Atdgen_extra_runtime.Oj_run.read_bool"
  | Int (_, Int o, Int) ->
      (match o with
         Int -> "Atdgen_extra_runtime.Oj_run.read_int"
       | Char -> "Atdgen_extra_runtime.Oj_run.read_int8"
       | Int32 -> "Atdgen_extra_runtime.Oj_run.read_int32"
       | Int64 -> "Atdgen_extra_runtime.Oj_run.read_int64"
       | Float -> "Atdgen_extra_runtime.Oj_run.read_number"
      )

  | Float (_, Float, Float _) -> "Atdgen_extra_runtime.Oj_run.read_number"

  | String (_, String, String) -> "Atdgen_extra_runtime.Oj_run.read_string"
...
let write_with_adapter adapter writer =
  ...
  | Some adapter_path ->
      ...
      [
        Line (
          sprintf "Atdgen_extra_runtime.Oj_run.write_with_adapter %s (" restore
        );
        Block writer;
        Line ")";
      ]
...
let rec make_writer ?type_constraint p (x : Oj_mapping.t) : Indent.t list =
  ...
  | List (loc, x, List o, List j) ->
      (match j with
         Array ->
           let write =
             match o with
               List -> "Atdgen_extra_runtime.Oj_run.write_list ("
             | Array -> "Atdgen_extra_runtime.Oj_run.write_array ("
           in
           [
             Line write;
       ...
       | Object ->
           ...
           let write =
             match o with
               List -> "Atdgen_extra_runtime.Oj_run.write_assoc_list ("
             | Array -> "Atdgen_extra_runtime.Oj_run.write_assoc_array ("
           in
           [
             Line write;
  ...
  | Option (_, x, Option, Option) ->
      [
        Line (sprintf "Atdgen_extra_runtime.Oj_run.write_%soption ("
  ...
  | Nullable (_, x, Nullable, Nullable) ->
      [
        Line "Atdgen_extra_runtime.Oj_run.write_nullable (";
        Block (make_writer p x);
        Line ")";
      ]
...
let unwrap_tmp_required_value ocaml_fname var =
  sprintf "(match !%s with Some x -> x \
           | None -> Atdgen_extra_runtime.Oj_run.missing_field p %S)"
    var ocaml_fname
...
let read_with_adapter adapter reader =
  ...
  | Some adapter_path ->
      ...
      [
        Line (
          sprintf "Atdgen_extra_runtime.Oj_run.read_with_adapter %s (" normalize
        );
        Block reader;
        Line ")";
      ]
...
let rec make_reader p type_annot (x : Oj_mapping.t) : Indent.t list =
  ...
  | Sum (_, a, Sum o, Sum j) ->
      ...
      let fallback_expr =
        [ Line "Atdgen_extra_runtime.Oj_run.invalid_variant_tag p x" ]
      in
      let cases =
      ...
      let read_tag =
        [
          ...
          Block [
            ...
            Block [
              Block [
                Line "match Atdgen_extra_runtime.Oj_run.read_string p lb with";
                Block cases1;
              ];
              Line ")";
            ];
  ...
  | List (loc, x, List o, List j) ->
      (match j with
         Array ->
           let read =
             match o with
               List -> "Atdgen_extra_runtime.Oj_run.read_list ("
             | Array -> "Atdgen_extra_runtime.Oj_run.read_array ("
           in
           [
             Line read;
       ...
       | Object ->
           ...
           let read =
             match o with
               List -> "Atdgen_extra_runtime.Oj_run.read_assoc_list ("
             | Array -> "Atdgen_extra_runtime.Oj_run.read_assoc_array ("
           in
           [
             Line read;
    ...
    | Some v ->
        let expr =
          if std then
            ...
            [
              Line "Atdgen_extra_runtime.Oj_run.read_until_field_value p lb;";
              Line "let x = (";
              Block [
                Block (make_reader p None v);
                Line ") p lb";
...
and make_tuple_reader p a =
  ...
  [
    ...
    Block [
      Line (sprintf
              "Atdgen_extra_runtime.Oj_run.missing_tuple_fields p !len %s);"
              req_fields);
    ];
  ]
