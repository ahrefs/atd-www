...
let rec get_reader_name
  ...
  let xreader s =
    if tagged then
      sprintf "Atdgen_extra_runtime.Ob_run.read_%s" s
    else
      sprintf "Atdgen_extra_runtime.Ob_run.get_%s_reader" s
  in
  match x with
...
let rec make_writer ~tagged deref (x : ob_mapping) : Indent.t list =
  ...
  | List (_, x, List o, List b) ->
      (match o, b with
         List, `Array ->
           let tag = get_biniou_tag x in
           [
             Line (sprintf "Atdgen_extra_runtime.Ob_run.write_%slist" un);
             Block [
               Line tag;
               Line "(";
       ...
       | Array, `Array ->
           let tag = get_biniou_tag x in
           [
             Line (sprintf "Atdgen_extra_runtime.Ob_run.write_%sarray" un);
             Block [
               Line tag;
               Line "(";
  ...
  | Nullable (_, x, Nullable, Nullable) ->
      [
        Line (sprintf "Atdgen_extra_runtime.Ob_run.write_%soption (" un);
...
and make_table_writer deref tagged list_kind x =
  ...
  let iter2 =
    match list_kind with
      List -> "Atdgen_extra_runtime.Ob_run.list_iter2"
    | Array -> "Atdgen_extra_runtime.Ob_run.array_iter2"
  in
...
let study_record ~ocaml_version fields =
  ...
  let check_bits =
    ...
    if k = 0 then []
      ...
      [ Line (sprintf "if %s then Atdgen_extra_runtime.Ob_run.missing_fields %s %s;"
    ...
    [
      ...
      Block [
        Line (sprintf "if Bi_io.read_tag ib <> %i then \
                       Atdgen_extra_runtime.Ob_run.read_error_at ib;"
                expected_tag);
        Inline body;
      ]
    ]
    ...
    [
      ...
      Block [
        Line (sprintf "if tag <> %i then \
                       Atdgen_extra_runtime.Ob_run.read_error () else"
                expected_tag);
        Block [
          Line "fun ib ->";
          Block body;
  ...
  if tagged then
    ...
    [
      ...
      Block [
        ...
        Block [
          Inline cases;
          Line "| _ -> Atdgen_extra_runtime.Ob_run.read_error_at ib"
        ]
      ]
    ]
  else
    ...
    [
      ...
      Block [
        Inline cases;
        Line "| _ -> Atdgen_extra_runtime.Ob_run.read_error ()"
      ]
    ]
...
let rec make_reader
        ...
        [
          ...
          Block [
            ...
            Block [
              ...
              Line "| _ -> Atdgen_extra_runtime.Ob_run.unsupported_variant h has_arg";
            ]
          ];
          Line ")"
        ]
  ...
  | List (loc, x, List o, List b) ->
      (match o, b with
         List, `Array ->
           let f =
             if tagged then "Atdgen_extra_runtime.Ob_run.read_list"
             else "Atdgen_extra_runtime.Ob_run.get_list_reader"
           in
           [
             Line (f ^ " (");
       ...
       | Array, `Array ->
           let f =
             if tagged then "Atdgen_extra_runtime.Ob_run.read_array"
             else "Atdgen_extra_runtime.Ob_run.get_array_reader"
           in
           [
             Line (f ^ " (");
             Block (make_reader deref ~ocaml_version ~tagged:false x);
       ...
       | list_kind, `Table ->
           ...
           let body2 =
             let f =
               match list_kind with
                 List -> "Atdgen_extra_runtime.Ob_run.read_list_value"
               | Array -> "Atdgen_extra_runtime.Ob_run.read_array_value"
             in
             [
               Line (f ^ " (");
...
and make_tuple_reader deref ~ocaml_version a =
  ...
  [
    Line "let len = Bi_vint.read_uvint ib in";
    Line (sprintf
            "if len < %i then Atdgen_extra_runtime.Ob_run.missing_tuple_fields len %s;"
...
and make_table_reader deref ~ocaml_version loc list_kind x =
  ...
  [
    ...
    Block [
      ...
      Line "let readers =";
      Block [
        Line "Atdgen_extra_runtime.Ob_run.array_init2 col_num ib (";
        Block [
          Line "fun col ib ->";
          Block [
...
let make_ocaml_files =
  Ox_emit.make_ocaml_files
    ~defs_of_atd_modules
    ~make_ocaml_intf:make_ocaml_biniou_intf
    ~make_ocaml_impl:make_ocaml_biniou_impl
    ~target:Biniou
